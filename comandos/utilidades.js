...
export const utilidades = async (c, mensagemInfoCompleta) => {
    ...
    try {
        switch (cmdSemPrefixo) {
            ...
            case "news":
                try {
                    await api.noticiasSobreContabilidade().then(async ({ resultado }) => {
                        await socket.reply(c, chatId, criarTexto(msgs_texto.utilidades.newsContabil.resposta, resultado), id)
                    }).catch(async (err) => {
                        if (!err.erro) throw err
                        await socket.reply(c, chatId, criarTexto(msgs_texto.geral.erro_api, command, err.erro), id)
                    })
