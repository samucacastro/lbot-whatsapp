...
export const noticiasSobreContabilidade = async () => {
    return new Promise(async (resolve, reject) => {
        try {
            let resposta = { sucesso: false }
            const html = await axios.get("https://www.contabeis.com.br/conteudo/contabil/");
            ...
            $('article').each((index, element) => {
                const article = {
                    ...
                    strongText: $(element).find('strong').text()
                };
                if (index == 10) return false
                newsArray.push(article);
            });
            let num = 0;
            const dados = newsArray.map((item) => {
                num++;
                return `${num}°: *${item.title}.*\n https://www.contabeis.com.br/${item.link} \nPostado ${item.timestamp} \n`
            }).join('\n');
            resposta = { sucesso: true, resultado: dados }
            resolve(resposta)

        } catch (err) {
            console.log(`API Scarper noticias contabeis - ${err.message}`)
            reject({ sucesso: false, erro: `Houve um erro na API Scarper noticias contabeis` })
        }
    })
}
...
export const respostaHercaiTexto = async (textoUsuario, usuario) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await herc.question({ content: textoUsuario, user: usuario }).then((respostaHercai) => {
